...
const MessagesInbox = () => {
  ...
  // Base styling constants for liquid glass
  const dropClass = "bg-white/40 backdrop-blur-md shadow-sm border border-white/60 rounded-3xl";
  const insetClass = "bg-white/50 border border-white/60 rounded-3xl";
  const btnClass = "p-2 rounded-full text-gray-500 hover:text-blue-500 hover:bg-white/60 transition-all";

  return (
    <div className="h-[calc(100vh-80px)] flex flex-col pt-6 font-inter overflow-hidden">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden max-w-7xl mx-auto w-full relative">
        ...
        <div className={`${(isMobileView && selectedMessage) ? 'hidden' : 'flex'} w-full md:w-1/3 lg:w-1/4 flex-col gap-4 h-full`}>
          <div className={`${dropClass} p-4 flex flex-col gap-4 h-full`}>
            ...
            <h2 className="text-2xl font-headline text-md-on-surface tracking-tight">Messages</h2>
            ...
            <div className={`${insetClass} flex items-center px-4 py-2`}>
              ...
              <input 
                ...
                className="bg-transparent border-none outline-none w-full text-sm text-md-on-surface"
                value={searchTerm}
                ...
                filteredMessages.map(msg => {
                  ...
                  return (
                    <div 
                      ...
                      className={`cursor-pointer p-3 rounded-2xl transition-all ${isSelected ? 'bg-white/80 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] border border-white' : 'hover:bg-white/50 border border-transparent'}`}
                    >
                      <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-2 overflow-hidden">
                          <div className={`w-2 h-2 shrink-0 rounded-full ${getStatusColor(msg.status)} ${isNew ? 'animate-pulse' : ''}`}></div>
                          <h4 className={`text-sm truncate ${isNew ? 'font-bold text-gray-900' : 'font-medium text-md-on-surface'}`}>{msg.name}</h4>
                        </div>
        ...
        <div className={`${(isMobileView && !selectedMessage) ? 'hidden' : 'flex'} flex-1 flex-col h-full bg-white/50 backdrop-blur-md rounded-3xl shadow-sm border border-white/60 overflow-hidden`}>
          
          {selectedMessage ? (
            <>
              {/* Chat Header */}
              <div className="h-16 shrink-0 border-b border-white/40 px-4 md:px-6 flex items-center justify-between bg-white/30 backdrop-blur-sm">
                <div className="flex items-center gap-4 overflow-hidden">
                  {isMobileView && (
                    <button onClick={() => setSelectedMessage(null)} className="p-2 text-gray-500 hover:text-md-on-surface shrink-0">
                      <ArrowLeft size={20} />
                    </button>
                  )}
                  ...
                  <div className="truncate">
                    <h3 className="font-bold text-md-on-surface text-sm truncate">{selectedMessage.name}</h3>
                ...
                <div className="flex items-center gap-2 shrink-0 ml-4">
                  <div className="hidden sm:flex bg-gray-100 p-1 rounded-lg mr-2">
                    <button onClick={() => handleStatusChange('read')} className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${selectedMessage.status === 'read' ? 'bg-md-surface shadow-sm text-md-on-surface' : 'text-gray-500 hover:text-md-on-surface'}`}>Read</button>
                    <button onClick={() => handleStatusChange('replied')} className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${selectedMessage.status === 'replied' ? 'bg-md-surface shadow-sm text-green-600' : 'text-gray-500 hover:text-md-on-surface'}`}>Replied</button>
                    <button onClick={() => handleStatusChange('archived')} className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${selectedMessage.status === 'archived' ? 'bg-md-surface shadow-sm text-purple-600' : 'text-gray-500 hover:text-md-on-surface'}`}>Archive</button>
                  </div>
                  <button onClick={() => handleDelete(selectedMessage._id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors bg-md-surface border border-gray-100 rounded-lg shadow-sm">
                    <Archive size={16} />
                  </button>
                </div>
              </div>
              ...
              {/* Chat Messages Area */}
              <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-transparent flex flex-col gap-6">
                
                {/* Date separator */}
                <div className="flex justify-center my-2">
                  <span className="px-3 py-1 bg-md-surface shadow-sm rounded-full text-xs font-medium text-gray-500">
                ...
                <div className="flex flex-col items-start max-w-[85%] sm:max-w-[75%]">
                  <div className="bg-white/70 backdrop-blur-sm text-md-on-surface p-4 rounded-3xl rounded-tl-sm shadow-sm border border-white">
                ...
                {selectedMessage.replies?.map((reply, idx) => (
                  <div key={idx} className="flex flex-col items-end self-end max-w-[85%] sm:max-w-[75%]">
                    <div className="bg-blue-50/80 backdrop-blur-sm text-md-on-surface p-4 rounded-3xl rounded-tr-sm shadow-sm border border-blue-100/50">
              ...
              {/* Chat Input Area */}
              <div className="p-4 bg-white/40 backdrop-blur-sm border-t border-white/50 flex items-end gap-3">
                <div className="flex-1 bg-gray-50 rounded-3xl border border-gray-200 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-100 transition-all flex flex-col relative overflow-hidden">
                  <textarea 
                    value={draftReply}
                    onChange={(e) => setDraftReply(e.target.value)}
                    placeholder="Type a quick, rough draft here..."
                    className="w-full bg-transparent p-4 outline-none resize-none min-h-[60px] max-h-[150px] text-sm text-md-on-surface custom-scrollbar"
                    disabled={isSending}
                    onKeyDown={(e) => {
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-transparent">
              ...
              <h2 className="text-2xl font-headline text-md-on-surface tracking-tight mb-2">WhatsApp Web... sort of!</h2>
