    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="relative">
        <textarea
          ...
          id={inputId}
          rows={rows}
          className={`w-full bg-white/50 backdrop-blur-sm shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] border-white/60 rounded-3xl px-4 py-3 text-md-on-surface placeholder-gray-500 focus:outline-none transition-all duration-300 border focus:ring-2 focus:bg-white/70 resize-y ${error ? 'border-md-error focus:border-md-error focus:ring-md-error' : 'focus:border-md-primary focus:ring-md-primary/20'}`}
          {...props}
        />
      </div>
      {error && (
